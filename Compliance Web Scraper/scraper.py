...
### This program takes links from 'target_urls.txt' and requests html data to see if 
### the website complies with various differetn privacy laws and policies. Data from
### program is written into "privacy_audit_report.csv" for further analysis. 
...
# writes data to the csv file named "privacy_audit_report.csv". Writes over file if it already exists
def save_to_report(data):
    with open('privacy_audit_report.csv', 'a', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        writer.writerow(data)

# uses rdap to gather registrant information from a url
...
# uses requests  to gather data from webpage in html
def audit_site(url):
    # used to bypass some anti-bot software
    headers = {
    ...
    try:
        ...
        # if request is sucessful, seperate the html into text, links, hrefs, and link text
        if response.status_code == 200:
            ...
            # CCPA/CPRA (California) legally requires: visible "Do Not Sell or Share" link, not just mentioned
            ...
            # GDPR (EU/UK) legally requires: cookie consent banner, DPO contact, right to erasure mention
            ...
            # PIPEDA (Canada) legally requires: named privacy officer, consent mechanism, access/correction rights
            ...
            # HIPAA (Healthcare) legally requires: Notice of Privacy Practices (NPP), PHI mention, patient rights
            ...
            # COPPA legally requires: parental consent mechanism, no collection from under 13 without consent
            ...
            # VCDPA (Virginia) legally requires: opt out of sale/targeted ads, right to appeal, data rights
            ...
            #General must have a linked privacy policy and terms of service
            ...
            # puts findings into a dictonary
            law_results = {
            ...
            # creates readable string for the findings
        ...
        # firewall blocked request attempt
        ...
        # other error during request to webpage
        else:
    ...
    # request times out after waiting for 5 sec for a response
    ...
    # DNS was not able to be resolved, thus not data was found
    ...
    # other errors sav
        ...
        save_to_report([url, "ERROR", e, registrar,
                        '', '', '', '', '',
                        '', '', '', '', '', '', '',
                        datetime.now().strftime("%Y-%m-%d %H:%M")])
# initiates program
def main():
    # opens the csv that data will be saved to
    with open('privacy_audit_report.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        # creates headers for the csv
